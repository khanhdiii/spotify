...
function ListItem({ image, name, href }: ListItemProps) {
  ...
  return (
    <button onClick={handleOnClick} className='relative group flex items-center rounded-md overflow-hidden gap-x-4 bg-neutral-100/10 hover:bg-neutral-100/20 transition pr-4' >
      ...
      <div className='absolute transition rounded-full opacity-0 flex items-center justify-center bg-green-500 p-4 drop-shadow-md right-5 group-hover:opacity-100 hover:scale-110'><FaPlay className="text-black" /></div>
    </button>
  );
}
